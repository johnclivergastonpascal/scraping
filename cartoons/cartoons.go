...
func getTotalPages() int {
	doc, err := getDocument("https://sololatino.net/series/")
	if err != nil {
...
func parseseries(url, year, titulo, genero, imagen string) (series, error) {
	doc, err := getDocument(url)
	if err != nil {
		return series{}, err
	}
...
func main() {
	...
	if _, err := os.Stat(jsonFile); err == nil {
		...
		if err == nil {
			defer file.Close()
			err = json.NewDecoder(file).Decode(&seriess)
			if err == nil {
				for _, s := range seriess {
					seriesMap[s.URL] = true
				}
				fmt.Printf("📁 Cargadas %d series desde el archivo existente.\n", len(seriess))
			}
		}
	}
	...
	for page := 1; page <= totalPages; page++ {
		pageURL := fmt.Sprintf("https://sololatino.net/genre_series/toons/page/%d/", page)
		...
		doc.Find("article").Each(func(i int, s *goquery.Selection) {
			...
			if link != "" {
				serieNueva, err := parseseries(link, year, titulo, genero, img)
				if err != nil {
				...
				if seriesMap[link] {
					...
					for i, s := range seriess {
						if s.URL == link {
							seriess[i].Temporadas = mergeTemporadas(seriess[i].Temporadas, serieNueva.Temporadas)
							seriess[i].Puntuacion = serieNueva.Puntuacion
							seriess[i].Sinopsis = serieNueva.Sinopsis
					...
					// Agregar nueva serie
					seriess = append(seriess, serieNueva)
					seriesMap[link] = true
